...
describe("Spec for signup route", () => {
    ...
    test("signup with valid invitation and credentials returns userId, json data, and nextUpdateKey", async () => {
        ...
        const req = MockReq({ iv, ticket, name, password });
        const res = MockRes();
        ...
        const signupResponse = { iv: 1, updateKey: 1, userData: ["{}", [], []] };
        expect(res.status).toHaveBeenCalledWith(200);
        expect(res.json).toHaveBeenCalledWith(signupResponse);

        const newUser = { _id: 1, credentials, data: 1, iv: 1, updateKey: 1 };
        ...
        expect(req.ciphers.generateIV).toHaveBeenCalledTimes(1);
        expect(req.ciphers.obscureUserData).toHaveBeenCalledWith(credentials, 1, ["{}", [], []]);
        expect(req.ciphers.obscureUpdateKey).toHaveBeenCalledWith(credentials, 1, 1);
    ...
    test("signup with invalid credentials returns errors", async () => {
        ...
        const noTicket = MockReq({ iv: 1, ticket: null, name, password });
        ...
        const noCred = MockReq({ iv: 1, ticket, name: null, password });
        ...
        const badTicket = MockReq({ iv: 1, ticket: "BEEF", name, password });
        ...
        const badcreds = MockReq({ iv: 1, ticket, name, password });
